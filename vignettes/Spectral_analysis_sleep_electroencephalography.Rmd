...
The [National Sleep Research Resource](https://sleepdata.org) (NSRR) is a repository of sleep databases containing  polysomnographic sleep recordings from various studies. The `Learn` dataset provides 3 records and their scoring for training purposes.
...
`rsleep` reads this particular format with the `read_events_profusion` function. The `plot_hypnogram` function then plots the hypnogram.
...
events <- rsleep::read_events_profusion(
...
The hypnogram show sleep stages over time using consecutive 30 seconds epochs. Data from the NSRR were manually scored by well-trained technicians according to the American Academy of Sleep Medicine manual @AASMScoringManual. Five sleep stages can be observed:
